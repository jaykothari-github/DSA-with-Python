...
class DoublyLinkedList:
    ...
    def set_value(self,index,value):
        var = self.get(index)       # Getting position where want to set
        if var:                     # if we get pos
            var.value = value       # changing value
            return True
        return False                # if we don't get pos

    def insert(self,index,value):

        if index == 0:                  # if want to add at index 0
            self.prepend(value)
            return True
        if index == self.length:        # if want to add at last index
            self.append(value)
            return True
        
        new_node = Node(value)          # Creating new node for setting
        var = self.get(index)           # reaching to require index

        if var is None:                 # if position is none or get method return none               
            return None
        
        temp = var.prev                 # finding index position and prev pos.
        temp.next = new_node            # setting prev's next to new node
        new_node.prev = temp            # setting new node's prev to prev node
        new_node.next = var             # setting new node's next to current point
        var.prev = new_node             # current point's prev to new node
        self.length += 1
        return True
    
    def remove(self,index):

        if index < 0 or index >= self.length:   # if index out of range
            return None
        
        if index == 0:                  # if index is 0
            return self.pop_first()
        
        if index == self.length -1:     # if index is last position
            return self.pop()
        
        var = self.get(index)           # getting index to remove
        pre = var.prev                  # finding index previous point
        pre.next = var.next             # setting previous next point to pointed index's next
        var.next.prev = pre             # setting pointed index's next's prev to previous pos
        var.next = None                 # detaching pointed index
        var.prev = None
        self.length -= 1
        return var
