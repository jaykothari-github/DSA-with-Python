...
class LinkedList:
    ...
    def binary_to_decimal(self):
        count = 0                           # setting counter
        node = self.head                    # setting start
        while node:                         # Until it is not end
            
            # dec count is if cur moves forward then - double previous value + current value
            count = (count*2) + node.value  
