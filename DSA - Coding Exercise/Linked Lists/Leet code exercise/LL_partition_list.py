...
class LinkedList:
    ...
    def partition_list(self,x):
        cur = self.head                 # Define start
        ll = LinkedList(0)              # define another LL for lesser num than x
        gl = LinkedList(0)              # define another LL for bigger or equal num to x

        while cur:
            if cur.value >= x:          # if cur value is big or eql to x than added into gl
                gl.append(cur.value)
            else:                       # otherwise it will add into ll
                ll.append(cur.value)
        ...
        # connecting gl at the end of ll for making a single LL
        temp = ll.head          
        while temp.next:
            temp = temp.next

        temp.next = gl.head.next

        self.make_empty()
        self.head = ll.head.next     # changing original LL's head with new ll's head
