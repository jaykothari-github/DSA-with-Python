...
class LinkedList:
    ...
    def remove_duplicates(self):
        ...
        # while cur:                              # checking current is not none
        #     run = cur

        #     while run.next:
        #         if run.next.value == cur.value: # if runner's next value is same as current
        #             run.next = run.next.next    # skipping that node
        #             self.length -= 1
        #         else:

        #             run = run.next              # next iteration
        #     cur = cur.next
        ...
        while cur:
            if cur.value in values:     # checking value in set
                pre.next = cur.next     # if present then skipping that node
                
            else:
                values.add(cur.value)   # if not than adding that into set
                pre = cur               # going for next iteration
            cur = cur.next
